# Helper functions for processing graphs and matrices associated with them,
# such as adjacency matrices and Laplacian matrices
...
def test_position_matrix(num_robots):
	"""Returns a num_robots x 2 matrix of random robot positions. """
...
def laplacian_to_adjacency(laplacian):
	"""Convert graph Laplacian matrix to adjacency matrix"""
	adj = -laplacian
...
def naive_averaging_laplacian(adjacency_matrix):
	"""An averaging matrix derived from a degree-based heuristic"""
...
def connect_nodes(adjacency_matrix, node_list):
	"""Augment a given adjacency matrix by connecting each pair of nodes in a given list."""
	n = adjacency_matrix.shape[0]
...
def personalized_page_rank(diffusion_matrix, alpha, seed_vector):
	"""Computes a PPR vector given a diffusion_matrix, diffusion parameter, and seed."""
	n = diffusion_matrix.shape[0]
